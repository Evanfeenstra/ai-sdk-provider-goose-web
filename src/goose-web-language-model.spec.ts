...
const createMockWebSocket = () => {
  const mockWebSocket = {
    ...
    onclose: undefined as ((event: { code: number; reason: string }) => void) | undefined,
...
describe('GooseWebLanguageModel', () => {
  ...
  describe('extractJson', () => {
    it('should extract valid JSON object', () => {
      const text = 'Here is the data: {"name": "John", "age": 30} and some more text';
  ...
  // Note: Integration tests with actual WebSocket mocking would be more complex
  // and would require careful async handling. For now, we test the core utility methods.
});