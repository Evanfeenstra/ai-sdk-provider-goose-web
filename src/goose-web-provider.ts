...
import { GooseWebLanguageModel, type GooseWebModelId } from './goose-web-language-model.js';
 ...
 * ```
 */
export function createGooseWeb(settings: GooseWebProviderSettings = {}): GooseWebProvider {
  const createModel = (
    modelId: GooseWebModelId,
    ...
    if (modelId !== 'goose' && typeof modelId !== 'string') {
      throw new NoSuchModelError({ modelId: String(modelId), modelType: 'languageModel' });
    }
