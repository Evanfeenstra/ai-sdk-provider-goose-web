...
describe("ID Consistency Tests", () => {
  ...
  describe("Text ID Consistency Logic", () => {
    ...
    it("should demonstrate tool ID consistency requirements", () => {
      ...
      const toolCallWithServerId = {
        type: "tool-call",
        toolCallId: serverProvidedId,
      };
      const toolResultWithServerId = {
        type: "tool-result",
        toolCallId: serverProvidedId,
      };
      ...
      const toolCallWithGeneratedId = {
        type: "tool-call",
        toolCallId: generatedId,
      };
      const toolResultWithGeneratedId = {
        type: "tool-result",
        toolCallId: generatedId,
      };

      expect(toolCallWithGeneratedId.toolCallId).toBe(
        toolResultWithGeneratedId.toolCallId
      );
  ...
  describe("Utility Methods", () => {
    it("should convert prompts correctly", () => {
      ...
      const multiPartResult = (model as any).convertPromptToText(
        multiPartPrompt
      );
    ...
    it("should extract JSON correctly", () => {
      ...
      const textWithJson =
        'Here is data: {"name": "John", "age": 30} more text';
