...
function handleToolRequest(data) {
  ...
  if (data.tool_name === "developer__shell" && data.arguments.command) {
    contentDiv.innerHTML = `<pre><code>${escapeHtml(
    ...
    contentDiv.innerHTML += `<div class="tool-param"><strong>path:</strong> ${escapeHtml(
      path
    )}</div>`;
    if (data.arguments.file_text) {
      contentDiv.innerHTML += `<div class="tool-param"><strong>content:</strong> <pre><code>${escapeHtml(
        data.arguments.file_text.substring(0, 200)
      )}${
        data.arguments.file_text.length > 200 ? "..." : ""
      }</code></pre></div>`;
    }
  } else {
...
function handleToolResponse(data) {
  ...
  if (data.is_error) {
    ...
    errorDiv.innerHTML = `<strong>Tool Error:</strong> ${escapeHtml(
